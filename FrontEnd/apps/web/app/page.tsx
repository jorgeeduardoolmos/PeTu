...
const sections = [
  { href: "/casos-de-uso",   icon: "👥", title: "Casos de uso",      desc: "23 casos organizados por rol: Coordinador, Entrenador, Jugador, Directivo y Familia.", color: "#63b3ff" },
  { href: "/reglas-negocio", icon: "📋", title: "Reglas de negocio", desc: "RN-01 a RN-27. Quién puede hacer qué, cuándo y bajo qué condiciones.",               color: "#ffd94d" },
  { href: "/wireframes",     icon: "📱", title: "Wireframes",        desc: "20 pantallas: splash, login, dashboards por rol, convocatoria, fixture y más.",        color: "#50c88c" },
  { href: "/arquitectura",   icon: "🏗️", title: "Arquitectura",      desc: "Diagrama de roles, flujos de datos y decisiones técnicas del sistema.",                color: "#f87171" },
  { href: "/roadmap",        icon: "🗓️", title: "Roadmap",           desc: "3 fases con 16 features. Estado actual de cada funcionalidad.",                        color: "#a78bfa" },
];
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 mb-12">
        {roles.map((r) => (
          <div key={r.name} className="bg-[#1a2744] border border-white/10 rounded-xl p-4 flex gap-3">
            ...
            <div>
              <div className="font-semibold text-sm mb-1" style={{ color: r.color }}>{r.name}</div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-12">
        {[{ n: "316", label: "Socios activos" }, { n: "6", label: "Categorías" }, { n: "23", label: "Casos de uso" }, { n: "27", label: "Reglas de negocio" }].map((s) => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {sections.map((s) => (
          <Link key={s.href} href={s.href} className="bg-[#1a2744] border border-white/10 rounded-xl p-5 hover:border-[#ffd94d44] transition-all group">
            <div className="flex items-start gap-3">
              <span className="text-2xl">{s.icon}</span>
              <div>
                <div className="font-semibold text-white group-hover:text-[#ffd94d] transition-colors mb-1">{s.title} →</div>
      ...
      <div className="mt-12 pt-8 border-t border-white/10 text-white/25 text-xs">
        Reunión kick-off: 15 mayo 2026 · Participantes: Andrés Lobo, Matías Maranzano, Matías Castagnola, Pela (dev), Jorge Olmos
      </div>
    </div>
  );
}
