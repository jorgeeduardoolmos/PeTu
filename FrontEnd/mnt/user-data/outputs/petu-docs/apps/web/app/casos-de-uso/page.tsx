  ...
  return (
    <div>
      <PageHeader
        label="Documentación"
        title="Casos de uso"
        subtitle="Qué puede hacer cada rol en la app. 23 casos organizados por actor y fase de desarrollo."
      />
          ...
          <div key={c.id} className="bg-[#1a2744] border border-white/10 rounded-xl p-5">
            ...
            {c.precondiciones && (
              <div className="text-xs text-white/35 mb-1">
                <span className="text-white/25">Precondición:</span> {c.precondiciones}
              </div>
            )}
            <div className="text-xs text-[#50c88c]">
              <span className="text-white/25">Resultado:</span> {c.resultado}
            </div>
          </div>
        ))}
      </div>

      <div className="mt-6 text-white/25 text-xs">{filtrados.length} casos de uso{rolActivo !== "Todos" ? ` para ${rolActivo}` : " en total"}</div>
    </div>
  );
}
